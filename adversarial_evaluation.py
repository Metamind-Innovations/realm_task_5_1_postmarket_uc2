...
def preprocess_groundtruth(csv_file_path):
    ...
    phenotype_columns = [col for col in groundtruth_df.columns if col not in ['Unnamed: 0', 'Sample']]
...
def evaluate_phenotype_predictions(groundtruth_file, predictions_file):
    ...
    if 'Sample' in groundtruth_df.columns and 'Sample' in predictions_df.columns:
        ...
        groundtruth_df, predictions_df = groundtruth_df.align(predictions_df, join='inner')
    
    phenotype_columns = [col for col in groundtruth_df.columns 
                         if col not in ['Unnamed: 0', 'Sample'] 
                         and col in predictions_df.columns]
...
def compare_evaluation_results(rwd_results_df, synthetic_results_df):
    ...
    phenotypes = set(rwd_results_df['Phenotype']).union(set(synthetic_results_df['Phenotype']))
    ...
    for phenotype in phenotypes:
        ...
        synthetic_row = synthetic_results_df[synthetic_results_df['Phenotype'] == phenotype]
        
        for metric in metric_columns:
            comparison[phenotype][metric] = {
                'rwd': float(rwd_row[metric].iloc[0]) if not rwd_row.empty else None,
                'synthetic': float(synthetic_row[metric].iloc[0]) if not synthetic_row.empty else None,
            }
            
            # Calculate difference if both values exist
            if comparison[phenotype][metric]['rwd'] is not None and comparison[phenotype][metric]['synthetic'] is not None:
                comparison[phenotype][metric]['difference'] = comparison[phenotype][metric]['synthetic'] - comparison[phenotype][metric]['rwd']
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Transform phenotype values in a CSV file")
    parser.add_argument("--groundtruth_file", required=True, help="Path to the input CSV file")
    parser.add_argument("--rwd_predictions_file", required=True, help="Path to the predictions CSV file")
    parser.add_argument("--synthetic_predictions_file", required=True, help="Path to the predictions CSV file")
    parser.add_argument("--output_file", default="artifacts/adversarial_evaluation.json", help="Path to the output JSON file")
    args = parser.parse_args()

    os.makedirs("artifacts", exist_ok=True)

    rwd_results_df = evaluate_phenotype_predictions(args.groundtruth_file, args.rwd_predictions_file)
    synthetic_results_df = evaluate_phenotype_predictions(args.groundtruth_file, args.synthetic_predictions_file)
